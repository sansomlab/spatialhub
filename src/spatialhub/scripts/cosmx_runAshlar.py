...
Outputs
-------
- A stitched OME-TIFF file of the field tiles (single channel, for check only).
...
def main():
    ...
    p.add_argument("--channel", type=int, default=0, help="Channel for alignment.")
    args = p.parse_args()
    print_arguments(args)

    out_csv = f"{args.out_pfx}_positions.csv"
    out_tiff = f"{args.out_pfx}_stitched.singlechannel.ome.tiff"
    ...
    aligner = EdgeAligner(reader, channel=args.channel)
    aligner.run()

    # Write out the stitched mosaic to an OME-TIFF file
    mosaic = Mosaic(aligner, aligner.mosaic_shape, channels=[args.channel])
