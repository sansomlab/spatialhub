...
if args.importAtomxMask:
    ...
    for dfk in df_ls:
        ...
        if len(dfk) >= 3:
            ...
            print("WARNING: cell boundaries defined by only", len(dfk), "coordinates! Excluding cell")
            print(dfk)
            excl_id = str(dfk['atomx_index'][0])
            cell_ids.remove(excl_id)
            i = i + 1

    #len(coords_ls)
    #coords_ls
    print(i, "cells out of", len(coords_ls), "were excluded from the segmentation mask (missing coordinates)")

    #if i/len(coords_ls) > 1/1000:
    #    raise Exception("More than 1 in 1000 cells from imported mask have missing coordinates.")
