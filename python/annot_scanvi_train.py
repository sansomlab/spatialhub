...
atlasPath = os.path.join("annot.dir/atlas.dir/", model_key + ".h5ad")
...
# scVI settings
scvi.settings.dl_num_workers = args.scviNumWorkers - 1
#scvi.settings.num_threads = 7


# scVI/scANVI model covariates
scvi_batch = df['batch_key'][0]
...
outDir = os.path.join("annot.dir/scanvi", model_key)
...
plot_covar = [scvi_batch] + [df['sample_key'][0]] + [df['donor_key'][0]] + scvi_categorical + scvi_continuous
...
if 'counts' in adata.layers.keys():
  print("'counts' layer already stored in input reference dataset")
...
if args.featureSet in ['hvg', 'both']:
    ...
    #elif 'hvg' in adata.uns.keys():
    #    hvg = adata.uns['hvg'].tolist()
    else:
        print("No HVG found in provided AnnData. Re-calculating with" + args.batchKey_hvg + "as batch_key.")
        adata = sc.pp.highly_variable_genes(adata,
                                            batch_key=args.batchKey_hvg,
...
# Make sure batch and categroical covariates are indeed categorical
...
sc.tl.umap(adata)
adata.write_h5ad(os.path.join(modelDir, model_key + '_feature-subset.h5ad'))
